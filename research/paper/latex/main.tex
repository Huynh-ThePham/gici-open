...
meant to provide.
We present an exact-in-window marginal at the optimizer's fixed linearization,
...
Directional Loewner diagnostics and a Ceres-fed end-to-end AR oracle are left
to future validation; accordingly, we claim neither directional dominance nor
trajectory equivalence to Ceres (Sec.~\ref{sec:limits}).
\end{abstract}
...
\begin{enumerate}
\item An algorithm that extracts the exact-in-window, fixed-linearization
  marginal ambiguity
...
File-mode UrbanNav configs disclose the retained \texttt{max\_age}=35\,s
setting and \texttt{relative\_frequency}=1.0; neither was
...
eigenvalues of the pair) are outside the evidence reported here; we therefore
make no directional-dominance claim (Sec.~\ref{sec:limits}).
...
UrbanNav Deep uses the session-aligned 5\,s HKKT base slice selected by the
repository runner (not the full-day 30\,s file).
Primary metric: unaligned raw ENU RMSE (E/N/U, yaw).
Sim(3) APE is used only when comparing to Chi Table~V.
Admissibility: Deep $n_{\mathrm{matched}}\ge14000$.
...
independent scenes. No claim is made that the proposed path reproduces
Ceres-fed end-to-end AR decisions.}
\end{table}
...
Medium is withheld from result tables because its 0\% NlFix regime does not
isolate AR effects from solver timing and ordering (Sec.~\ref{sec:limits}).
...
\paragraph{Boundary of the end-to-end evidence.}
This paper does not claim that proposed AR decisions and trajectories match a
Ceres-fed AR oracle. Its primary validation is covariance-level agreement and
runtime; UrbanNav results quantify secondary system impact.
...
\begin{itemize}
\item \emph{Exactness scope.} Frobenius match at $10^{-3}$ covers 90.1\% of
  usable epochs. Without Loewner, per-diagonal, and worst-direction tests, the
  evidence does not establish elementwise equivalence or directional dominance.
\item \emph{No Ceres e2e equivalence claim.} Covariance-level agreement does
  not imply identical AR decisions or trajectories on UrbanNav.
\item \emph{Medium withheld.} At 0\% NlFix, arm-to-arm differences can reflect
  solver timing, WlFix, or ordering rather than the tested AR covariance.
\item \emph{Soft real-time.} The 50\,ms figure is a covariance-query budget at
  $\sim$1\,Hz AR; 4.3\% exceed it (max 97.5\,ms). It is not a reported bound on
  total optimize-plus-covariance latency.
\item \emph{Statistics.} Three repeats of one Deep sequence are not three
  independent environments.
\item \emph{Decision-layer urban mitigations} are outside this letter. In
  particular, the Deep horizontal heavy tail remains unresolved. Because it is
  consistent with residual multipath on tracked (line-of-sight) signals rather
  than covariance-scale error, a GNSS-independent consistency-based float
  weighting is reserved for a separate follow-on study.
\end{itemize}
...
usable epochs.
Directional Loewner diagnostics, a Ceres end-to-end AR oracle, and deterministic
float-path isolation would support stronger future claims, but are not assumed
by the bounded claims in this paper.
